...
const SERIES_EPISODES_PATTERN = /^(.+?):\s*(\d+)\s*Folgen?\s*geschaut$/i;
const MOVIE_PATTERN = /^(.+?)\s*geschaut$/i;
const BOOK_CHAPTERS_PATTERN = /^(.+?):\s*(\d+)\s*Kapitel\s*gelesen$/i;
const BOOK_PAGES_PATTERN = /^(.+?):\s*(\d+)\s*Seiten\s*gelesen$/i;
...
export function parseLine(line: string): ParsedLine | null {
  ...
  if (!trimmed || isDrawingEntry(trimmed)) return null;
  ...
  if (seriesMatch) {
    return { category: 'series', name: seriesMatch[1].trim(), episodes: Number(seriesMatch[2]) };
  }

  const bookChaptersMatch = trimmed.match(BOOK_CHAPTERS_PATTERN);
  if (bookChaptersMatch) {
    return { category: 'book', name: bookChaptersMatch[1].trim(), chapters: Number(bookChaptersMatch[2]) };
  }
  const bookPagesMatch = trimmed.match(BOOK_PAGES_PATTERN);
  if (bookPagesMatch) {
    return { category: 'book', name: bookPagesMatch[1].trim(), pages: Number(bookPagesMatch[2]) };
  }
